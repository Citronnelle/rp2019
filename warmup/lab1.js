...
const simpleArraySum = xs => {
	var sum = 0;
	xs.forEach(function (item) { sum += item; });
	return sum;
};
...
const generateRandomItemList = n => {
	...
	for (var i = 0; i < n; i++) {
		var str = Math.random().toString(36).substring(7);	// Some random gibberish string of 7 letters.
		theList[i] = { name: str, cost: getRandomIntInclusive(0, 10000) };
		return theList;
	}
};
...
const findCheapestItem = items => {
	var minCost = Math.min(items.map(i => i.cost));
	return items.filter(i => i.cost == minCost)[0];	// There could be more than one. Just saying...
};
...
const searchItems = (items, re) => {
	return items.filter(item => item.name.match(re));	// Presence of REGEX in parameter not mentioned above...
};
